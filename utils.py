...
def build_gebco_interpolator(path: str) -> RegularGridInterpolator:
    """Build an interpolator (lat, lon) -> elevation from a GEBCO NetCDF file.
    ...
    RegularGridInterpolator
        Interpolator that takes points [[lat, lon], ...] and returns elevation.
    """
    ds = xr.open_dataset(path)

    # GEBCO usually uses variables 'lon', 'lat', and 'elevation'
    lon = ds["lon"].values
    lat = ds["lat"].values
    # Data is typically stored as (lat, lon)
    z = ds["elevation"].values

    # Ensure latitude increases
    if lat[1] < lat[0]:
        lat = lat[::-1]
        z = z[::-1, :]
...
def interpolate_gebco_on_grid(nc_path: str, X: np.ndarray, Y: np.ndarray) -> np.ndarray:
    """Interpolate GEBCO elevation onto a given (X,Y) grid.
    ...
    nc_path : str
        Path to the GEBCO NetCDF file.
    X, Y : np.ndarray
        2D arrays (same shape) with longitudes (X) and latitudes (Y).

    Returns
    -------
    np.ndarray
        2D array with interpolated elevation values on the X,Y grid.
    """
    interp = build_gebco_interpolator(nc_path)
    pts = np.stack([Y.ravel(), X.ravel()], axis=-1)  # order: (lat, lon)
    Z = interp(pts).reshape(X.shape)
    return Z


def get_frame_count(output_path: str) -> int:
    """Get the number of frames in a Clawpack output directory.
    ...
    -------
    int
        Number of frames available in the output directory.
    """
...
def load_bathymetry_and_meshgrid(
    ...
    """Load the bathymetry meshgrid from a Clawpack output directory.
    ...
    -------
    Tuple[np.ndarray, Tuple[np.ndarray, np.ndarray]]
        2D array with bathymetry values and coordinates.
    """
    X, Y = np.load(os.path.join(output_path, "coord_meshgrid.npy"), allow_pickle=True)
    bathymetry = np.load(os.path.join(output_path, "bathymetry.npy"))
    return bathymetry, (X, Y)


def read_solutions(
    outdir: str = "_outdir", frames_list: List[int] | None = None
) -> Dict[str, Any]:
    ...
    if frames_list is None:
        n_sol = get_frame_count(outdir)
        frames_list = list(range(n_sol))
    ...
    for i, f in enumerate(frames_list):
        ...
        if solutions is None:
            solutions = np.empty(
                (len(frames_list), *sol.state.q.shape)
            )  # (n_frames, num_eqn, nx, ny)
        solutions[i] = sol.q
    bathymetry, meshgrid = load_bathymetry_and_meshgrid(outdir)
    result["bathymetry"] = bathymetry
    result["meshgrid"] = meshgrid
    result["solutions"] = solutions if solutions is not None else np.array([])
    return result
