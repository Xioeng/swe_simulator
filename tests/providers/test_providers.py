...
def test_initial_condition_abstract_class_cannot_be_instantiated():
    """InitialConditionProvider should not be instantiable."""
    with pytest.raises(TypeError):
        InitialConditionProvider()


def test_gaussian_hump_shape(basic_config):
...
def test_gaussian_hump_values(basic_config):
...
def test_wind_provider_abstract_class_cannot_be_instantiated():
...
def test_bathymetry_provider_abstract_class_cannot_be_instantiated():
    """BathymetryProvider should not be instantiable."""
    with pytest.raises(TypeError):
        BathymetryProvider()


def test_flat_bathymetry_shape(basic_config):
...
def test_flat_bathymetry_values(basic_config):
...
def test_sloping_bathymetry(basic_config):
